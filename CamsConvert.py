...
def processNight(night_data_dir, config, detection_results=None, nodetect=False):
    ...
    if (not nodetect):
        ...
        print("config.cams_code {}".format(config.cams_code))
        print("Platepar {}".format(platepar))
        if (int(config.cams_code) > 0) and (platepar is not None):

            log.info('Generating a CAMS FTPdetectinfo file...')

            # Write the CAL file to disk

            config.cams_code = int(config.cams_code)
            cal_file_name = writeCAL(night_data_dir, config, platepar)
            print("Cal file name {}".format(cal_file_name))
    ...
    captured_night_dir = night_data_dir

    # START HACK
    # N. Moskovitz
    #   -report captured data to SETI server as a single zip file
    #   -Add check on presence of FTPdetectinfo before trying SETI transfer
    #   -Fix parsing of CAMS file names to avoid CAMS ID = date string
    #
    #
    # Send data to SETI server with anonymous FTP transfer
    # transfer FTPdetectinfo*txt and CAL*txt from CapturedFiles directory
    #
    # create copy of FTPdetectinfo with new name, zip with cal file
    send_to_seti = True
    if send_to_seti:
        log.info('Start reporting of files to SETI...')
        print("Config.cams_code {}".format(config.cams_code))
        cams_str = str(config.cams_code).strip()
        detect_str = '/FTPdetectinfo_00' + cams_str + '*txt'
        detect_file = glob.glob(captured_night_dir + detect_str)
        print("Search for {}".format(captured_night_dir + detect_str))
        print("len(detect_file) {}".format(len(detect_file)))
        if len(detect_file) > 0:
            new_detect_file = [detect_file[0].replace('FTPdetectinfo', 'RTPdetectinfo')]
            cp_cmd = 'cp ' + detect_file[0] + ' ' + new_detect_file[0]
            subprocess.call(cp_cmd, shell=True)

            cal_str = '/CAL_00' + cams_str + '*txt'
            cal_file = glob.glob(captured_night_dir + cal_str)
            print("cal_file {}".format(cal_file))
            local_files = new_detect_file + cal_file
            for file in local_files:
                log.info('   File for zipping: ' + file)

            # write to zip with date and time info in file name
            date_start = new_detect_file[0].find('00' + cams_str) + len(cams_str) + 3
            print("date_start {}, cams_str {}".format(date_start, cams_str))
            time_start = date_start + 9
            ymd_pack = new_detect_file[0][date_start:date_start + 8]
            ymd_str = ymd_pack[0:4] + '_' + ymd_pack[4:6] + '_' + ymd_pack[6:]
            hms_pack = new_detect_file[0][time_start:time_start + 6]
            hms_str = hms_pack[0:2] + '_' + hms_pack[2:4] + '_' + hms_pack[4:6]

            zip_file = captured_night_dir + '/' + ymd_str + '_00' + cams_str + '_' + hms_str + '_01.zip'
            log.info('   Zipping files to ' + zip_file)
            with ZipFile(zip_file, 'w') as zip:
                # write detect and cal files to single zip
                for file in local_files:
                    zip.write(file, arcname=ntpath.basename(file))

    log.info('ALL LOWELL REPORTING COMPLETE!')
    #
    # END HACK
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument('-s', '--cams_code', nargs=1, metavar='CAMS_CODE', type=str, \
        help="Path to a config file which will be used instead of the default one.")
    ...
    config.cams_code = int(cml_args.cams_code[0])

    print("Working with {}".format(config.cams_code))
